...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Barlow:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400&family=Barlow+Condensed:wght@400;500;600;700&family=DM+Mono:wght@400;500&display=swap');

*{font-family:'Barlow',sans-serif!important;box-sizing:border-box}
.stApp{background:#09090b;min-width:100%}
...
.h-eyebrow{
  ...
  color:#3f3f46;margin-bottom:14px;font-family:'DM Mono',monospace!important;
  display:flex;align-items:center;gap:16px;
}
.h-eyebrow::before{content:'';width:28px;height:1px;background:#27272a}
.h-top{display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:20px}
.h-left{}
.h-title{
  font-size:52px;font-weight:700;color:#fafafa;margin:0;
...
.h-tagline{
  font-size:13px;font-weight:400;color:#52525b;margin:8px 0 0;
  font-family:'DM Mono',monospace!important;letter-spacing:.02em;
}
.h-pills{display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:24px}
.pill{
  background:transparent;border:1px solid #27272a;border-radius:3px;
  padding:7px 14px;font-size:10px;color:#52525b;
  font-family:'DM Mono',monospace!important;white-space:nowrap;
  letter-spacing:.04em;
}
.pill strong{color:#d4a843;font-weight:500}
.pill.active{border-color:#d4a843;color:#d4a843}
...
.sec{
  ...
  color:#3f3f46;margin-bottom:12px;display:flex;align-items:center;gap:12px;
  font-family:'DM Mono',monospace!important;
}
.sec::after{content:'';flex:1;height:1px;background:#27272a}

.pnl{
  background:#0f0f12;border:1px solid #27272a;border-radius:6px;
...
.stNumberInput input:focus{border-color:#d4a843!important;outline:none!important;box-shadow:none!important}
...
/* status card */
.sc{border-radius:4px;padding:18px 22px;margin:14px 0;display:flex;align-items:center;gap:14px}
...
.sd{font-size:12px;color:#52525b;font-family:'DM Mono',monospace!important;letter-spacing:.02em}

/* metric cards */
.mg{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0}
.mc{background:#0f0f12;border:1px solid #27272a;border-radius:4px;padding:16px 18px}
.mv{
...
.ml{font-size:9px;color:#3f3f46;text-transform:uppercase;letter-spacing:.2em;font-weight:600}
.ms{font-size:10px;color:#27272a;margin-top:3px;font-family:'DM Mono',monospace!important}

/* breakdown table */
.bd{background:#0f0f12;border:1px solid #27272a;border-radius:4px;overflow:hidden;margin:10px 0}
.br{display:flex;justify-content:space-between;align-items:center;padding:10px 16px;border-bottom:1px solid #18181b}
.br:last-child{border-bottom:none}
.bk{font-size:10px;color:#52525b;font-family:'DM Mono',monospace!important;letter-spacing:.04em}
...
.ir{display:flex;justify-content:space-between;padding:10px 16px;border-bottom:1px solid #18181b;font-size:11px}
.ir:last-child{border-bottom:none}
.ik{color:#52525b;font-family:'DM Mono',monospace!important;letter-spacing:.04em}
...
def col(pct): return "#22c55e" if pct<60 else "#f59e0b" if pct<85 else "#ef4444"
def bdg(val,ref):
    r=val/ref
...
def draw_roulette(pct,wear,rul):
    bc=col(pct)
    fig,ax=plt.subplots(figsize=(5.6,4.6),facecolor="#09090b")
    ...
    ax.add_patch(plt.Circle((0,0),1.45,fill=False,edgecolor="#d4a843",linewidth=1.5,alpha=0.7,zorder=2))
    ax.add_patch(plt.Circle((0,0),1.31,fill=False,edgecolor="#d4a843",linewidth=0.5,alpha=0.2,zorder=2))

    # segments — alternating red/black casino style with zone brightness
    n=36
    for i in range(n):
        t1=180-(i*180/n); t2=180-((i+1)*180/n)
        if i<int(n*0.60):   c1="#7a1515" if i%2==0 else "#1a1a1e"
        elif i<int(n*0.85): c1="#6b4e00" if i%2==0 else "#1a1a1e"
        else:                c1="#5c1212" if i%2==0 else "#1a1a1e"
        ax.add_patch(Wedge((0,0),1.28,t2,t1,width=0.15,
                           facecolor=c1,edgecolor="#09090b",linewidth=1.8,alpha=0.95))

    # segment numbers every 6th
    ...
    for i in range(n):
        ...
        ax.add_patch(plt.Circle((1.29*np.cos(ang),1.29*np.sin(ang)),
                                0.018,color="#d4a843",zorder=6,alpha=0.85))

    # inner separator ring
    ax.add_patch(plt.Circle((0,0),1.11,fill=False,edgecolor="#27272a",linewidth=1))

    # gauge face
    ax.add_patch(Wedge((0,0),1.07,0,180,width=0.46,
                       facecolor="#0f0f12",edgecolor="#27272a",linewidth=1))

    # subtle zone wash
    for z0,z1,zc in [(0,.60,"#22c55e"),(.60,.85,"#f59e0b"),(.85,1.0,"#ef4444")]:
        ax.add_patch(Wedge((0,0),1.06,180-z1*180,180-z0*180,
                           width=0.44,facecolor=zc,edgecolor="none",alpha=0.055))

    # progress fill
    if pct>0:
        ax.add_patch(Wedge((0,0),0.99,180-pct*1.8,180,
                           width=0.30,facecolor=bc,edgecolor="none",alpha=0.16))
        # bright leading edge
        ax.add_patch(Wedge((0,0),0.99,180-pct*1.8,180-pct*1.8+1.8,
                           width=0.30,facecolor=bc,edgecolor="none",alpha=0.8))

    # tick marks + labels
    for pm,lbl,tc in [(0,"0","#3f3f46"),(60,"60%","#22c55e"),(85,"85%","#f59e0b"),(100,"100%","#ef4444")]:
        ang=np.radians(180-pm*1.8)
        ax.plot([0.64*np.cos(ang),0.74*np.cos(ang)],[0.64*np.sin(ang),0.74*np.sin(ang)],
                color=tc,lw=1.8,alpha=0.9,zorder=7)
        ax.text(0.81*np.cos(ang),0.81*np.sin(ang),lbl,ha='center',va='center',
                fontsize=7,color=tc,fontfamily='monospace',fontweight='bold')

    # needle shadow then needle
    na=np.radians(180-pct*1.8)
    nx,ny=0.58*np.cos(na),0.58*np.sin(na)
    ax.plot([0,nx],[0,ny],color="#000",lw=5,zorder=8,solid_capstyle='round',alpha=0.6)
    ...
    # center hub
    ax.add_patch(plt.Circle((0,0),0.060,color="#27272a",zorder=10))
    ax.add_patch(plt.Circle((0,0),0.040,color="#d4a843",zorder=11))
    ax.add_patch(plt.Circle((0,0),0.018,color="#09090b",zorder=12))

    # readout
    ax.text(0,0.26,f"{pct:.1f}%",ha='center',va='center',fontsize=22,
            fontweight='700',color=bc,fontfamily='monospace',
            path_effects=[pe.withStroke(linewidth=4,foreground="#0f0f12")])
    ax.text(0,0.10,"LIFE CONSUMED",ha='center',va='center',
            fontsize=6.5,color="#3f3f46",fontfamily='monospace',fontweight='600')

    # dividers
    ax.plot([-0.62,0.62],[-0.19,-0.19],color="#27272a",lw=0.8)
    ax.plot([0,0],[-0.19,-0.72],color="#27272a",lw=0.8)

    # stats below
    ax.text(-0.55,-0.34,f"{wear:.1f}",ha='center',va='center',
            fontsize=16,fontweight='700',color="#d4a843",fontfamily='monospace')
    ax.text(-0.55,-0.48,"µm wear",ha='center',va='center',
            fontsize=7,color="#3f3f46",fontfamily='monospace')
    rc="#22c55e" if rul>10 else "#f59e0b" if rul>3 else "#ef4444"
    ax.text(0.55,-0.34,f"{rul}",ha='center',va='center',
            fontsize=16,fontweight='700',color=rc,fontfamily='monospace')
    ax.text(0.55,-0.48,"cycles left",ha='center',va='center',
            fontsize=7,color="#3f3f46",fontfamily='monospace')
...
def draw_wear_chart(cycle,wear,rul,wr,sf):
    ...
    ax.scatter([cycle],[wear],color=bc,s=70,zorder=6,edgecolors="#0f0f12",lw=2)
    ax.text(cycle+0.3,wear+13,f"{wear:.0f}µm",color=bc,fontsize=8.5,
            fontfamily="monospace",fontweight='500')
    if rul>0:
        ax.scatter([cycle+rul],[THRESHOLD],color="#ef4444",s=50,zorder=6,marker="x",lw=2.5)
        ax.text(cycle+rul+0.2,THRESHOLD-24,"FAIL",color="#ef4444",
                fontsize=7.5,fontfamily="monospace",alpha=0.8)
    ...
    ax.set_title("Wear Trajectory",color="#52525b",fontsize=9,
...
def draw_influence(speed,feed,doc,vm,spindle,roughness):
    ...
    fig,ax=plt.subplots(figsize=(10.5,2.8),facecolor="#0f0f12")
    ax.set_facecolor("#0f0f12")
    ...
    for bar,v,p,c in zip(bars,vals,pcts,cols):
        ax.text(v+0.004,bar.get_y()+bar.get_height()/2,
                f"{p:.0f}%",va="center",color=c,fontsize=9,
                fontfamily="monospace",fontweight='500')
    ax.set_xlabel("Influence Score",color="#52525b",fontsize=9)
    ax.tick_params(colors="#52525b",labelsize=9)
    for sp in ax.spines.values(): sp.set_edgecolor("#27272a")
    ax.set_title("Parameter Influence on Wear",color="#52525b",fontsize=9,
...
st.markdown(f"""
<div class="hero">
  <div class="h-eyebrow">Predictive Maintenance System · Bachelor Thesis Project · IIT</div>
  <div class="h-top">
    <div class="h-left">
      <div class="h-title">Tool<em>Sense</em></div>
      <div class="h-tagline">machine learning–based tool wear & remaining useful life estimator</div>
    </div>
  </div>
  <div class="h-pills">
    ...
    <div class="pill">Models <strong>Gradient Boosting · XGBoost</strong></div>
    <div class="pill {'active' if models_loaded else ''}">{'✓ ML Model Active' if models_loaded else '⚠ Fallback Mode'}</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with c1:
    ...
    with s1:
        ...
        roughness=st.number_input("Surface Roughness — µm",value=1.4,step=0.1,format="%.1f")
    with s2:
...
with c2:
    if clicked:
        ...
        elif pct<85: sc,icon,lbl="s-w","▲","Warning — Schedule Replacement"; desc=f"Elevated wear detected. Replace within {rul} cycles to avoid unexpected failure."
        else:        sc,icon,lbl="s-c","■","Critical — Replace Immediately"; desc=f"Tool approaching failure. Only {rul} cycles remaining. Stop operation now."
        ...
        <div class="mg">
          ...
          <div class="mc">
            ...
            <div class="ml">Tool Life Consumed</div>
            <div class="ms">{100-pct:.1f}% service life remaining</div>
          </div>
        ...
        rows=[
            ("speed_rpm",          f"{speed} rpm",         *bdg(speed,1200)),
        ...
        html="".join([
            ...
            f'<div><span class="bv">{r[1]}</span>'
            f'<span class="bg_ {r[3]}">{r[2]}</span></div></div>'
            for r in rows])
        st.markdown(
            f'<div class="sec" style="margin-top:14px">Feature Breakdown</div>'
            f'<div class="bd">{html}</div>',
            unsafe_allow_html=True)

        rec=("Continue operation normally. Schedule next inspection in 10 cycles." if pct<60
        ...
        <div style="background:#0f0f12;border:1px solid #27272a;border-left:3px solid {bc};
             border-radius:4px;padding:16px 20px;margin-top:12px">
          <div style="font-size:9px;color:#3f3f46;text-transform:uppercase;letter-spacing:.22em;
               font-family:'DM Mono',monospace;margin-bottom:6px">Maintenance Recommendation</div>
          <div style="font-size:15px;color:#e4e4e7;font-weight:500;letter-spacing:-.2px">{rec}</div>
          <div style="font-size:10px;color:#27272a;margin-top:8px;font-family:'DM Mono',monospace">
          {'Gradient Boosting (wear) · XGBoost (RUL) · GroupKFold K=10 · Holdout R² 0.97' if models_loaded else 'Physics fallback active — upload PKL files to enable ML'}</div>
        </div>""", unsafe_allow_html=True)

    else:
        st.markdown("""
        <div style="padding:100px 20px;text-align:center">
          <div style="font-size:10px;color:#27272a;letter-spacing:.22em;text-transform:uppercase;
               font-family:'DM Mono',monospace;margin-bottom:16px">Awaiting Input</div>
          <div style="font-size:32px;font-weight:700;color:#18181b;letter-spacing:-1.5px;
               font-family:'Barlow Condensed',sans-serif">Configure parameters.<br>Run prediction.</div>
        </div>
        <div class="sec">Model Performance Summary</div>
        <div class="ic">
          ...
          <div class="ir"><span class="ik">holdout_wear_r2</span><span class="iv">0.9756</span></div>
          <div class="ir"><span class="ik">holdout_rul_r2</span><span class="iv">0.9749</span></div>
